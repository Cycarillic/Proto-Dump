...
void PrintMethodValue(int val,ofstream& out) {
    cout << COL_VALUE << hex << "-HEX >> " << COL_NUM << "0x" << setw(2) << val << COL_VALUE << " | DEC >> " << dec << COL_NUM << val << COL_RESET;
...
void PrintMethodLen(int val,ofstream& out) {
    ...
    cout << COL_VALUE << hex << "-HEX >> " << COL_NUM << "0x" << setw(2) << val << COL_VALUE << " | DEC >> " << dec << COL_NUM << val << " | " << COL_ASCII << static_cast<char>(b) << COL_RESET;
...
float get_i32_val(ifstream& in,ofstream& out) {
    ...
    cout << "\n\t |~ " << "Bytes in I32 >> " << COL_HEX << "0x" << hex << setw(8) << be << COL_RESET;
    cout << "\n\t |~ " << COL_ASCII << "i32 value: " << dec << result << COL_RESET;

    out << "\n\t\t |~ " << "Bytes in I32 >> " << "0x" << hex << setw(8) << be;
    out << "\n\t\t |~ " << "i32 value: " << dec << result << "\n";

    return result;
}

//Unused Sub-Blocks codes
/*
void get_len_value(ifstream& in, int len, ofstream& out) {
    ...
    while (byte_count < len) {
        ...
        int current_offset = offset;  // save current offset for printing
        offset++;  // increment offset since we read a byte

        PrintMethodOffset(current_offset, out);
        cout << "-" << COL_LEN << " len " << COL_RESET << "- (" << COL_VALUE << "byte " << COL_NUM << byte_count << COL_RESET << ") >> ";
        out << "-" << " len " << "- (" << "byte " << byte_count << ") >> ";
        PrintMethodValue(b, out);

        // For ASCII printing, convert bytes >127 to '.'
        char ascii_char = (b > 127) ? '.' : static_cast<char>(b);

        if (wire_type == 0b000 and b != 0x00) {  // varint inside len
            cout << " | WIRE TYPE >> VARINT";
            out << " | WIRE TYPE >> VARINT";

            int next_val = in.get();
            if (next_val == EOF) {
                ERROR_HANDLE(1, "EOF while reading varint inside len");
                break;
            }
            PrintMethodOffset(offset, out);
            PrintMethodValue(next_val, out);
            cout << " (varint value)";
            out << " (varint value)";
            ...
            byte_count++;  
            ascii += ascii_char;  
            ascii += (next_val > 127 ? '.' : static_cast<char>(next_val));  

        } else if (wire_type == 0b101) {  // i32 inside len
            cout << " | WIRE TYPE >> I32";
            out << " | WIRE TYPE >> I32";

            float i32_val = get_i32_val(in, out);
            
            byte_count += 4;  
            
            ascii += to_string(i32_val);
...
int main(int argc, char* argv[]) {
    ...
    if (!in) {
        ...
        cout << COL_ERROR << argv[1] << COL_RESET;
        return 1;
    }
    ...
    for (int a = 0; a < argc - 1; ++a) {
        if (string(argv[a]) == "-o" or "--outfile") {
            out_file_name = argv[a + 1];
        }else if (string(argv[a]) == "-s" or "--stop") {
            if (a + 1 < argc) {
        ...
        }else if (string(argv[a]) == "-h" or "--help"){
                cout << R"(
PROTO DUMP HELP : 
  ...
  - ASCII decoding is shown for length fields.
    ...
    while ((byte = in.get()) != EOF) {
        ...
        if (byte != 0x00){
            ...
            cout << COL_OFFSET << "0x" << uppercase << setw(6) << setfill('0') << hex << old_offset << COL_RESET << " ~ "
            ...
            if (bit3 == 0b000) {  // varint
                ...
                PrintMethodOffset(offset,out);
                //Output the length
                cout << COL_LEN << "- Length >> " << COL_NUM<< len << COL_RESET;
                out << "- Length >> " << len ;
                offset++;
    ...
    cout << COL_NUM << endl << argv[1] << " Successfully DUMPED as " << out_file_name << COL_RESET ;
